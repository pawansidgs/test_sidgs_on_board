...
var environment = config['test_sidgs_on_board'].env;
...
console.log('&test_sidgs_on_board api: [' +  domains[environment] + ', ' + config['test_sidgs_on_board'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['test_sidgs_on_board'].app, config['test_sidgs_on_board'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['test_sidgs_on_board'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
